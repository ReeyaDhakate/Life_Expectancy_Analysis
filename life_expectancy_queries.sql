-- =========================================================
-- Life Expectancy Analysis — MySQL Queries
-- Table: life
...
--          diphtheria, hivaids, gdp, population, thinness_119_years,
--          thinness_59_years, income_composition_of_resources,
--          schooling, life_expectancy_band, log_gdp,
--          vaccination_coverage_avg, child_mortality_combined,
--          thinness_avg, high_health_spend, is_developed
-- =========================================================

-- =========================
-- 0) BASIC ROW / SANITY CHECKS
-- =========================

SELECT COUNT(*) AS total_rows FROM life;

SELECT COUNT(DISTINCT country) AS total_countries FROM life;

SELECT MIN(year) AS min_year, MAX(year) AS max_year FROM life;
...
WITH first_year AS (
    ...
    FROM life
    WHERE year = (SELECT MIN(year) FROM life)
),
last_year AS (
    SELECT country, life_expectancy AS le_end, year AS end_year
    FROM life
    WHERE year = (SELECT MAX(year) FROM life)
)
SELECT
    f.country,
    f.le_start,
    l.le_end,
    ROUND(l.le_end - f.le_start, 2) AS change
FROM first_year f
...
SELECT f.country, f.le_start, l.le_end, ROUND(l.le_end - f.le_start, 2) AS change
FROM first_year f
JOIN last_year l ON f.country = l.country
WHERE f.le_start IS NOT NULL AND l.le_end IS NOT NULL
ORDER BY change DESC
...
WHERE status = 'Developing'
  AND year = (SELECT MAX(year) FROM life)
...
-- =========================================================
-- Note: MySQL has no built-in CORR() aggregate (unlike PostgreSQL).
-- Pearson correlation can be computed manually using the standard formula,
-- or approximated via bucket/tier analysis as shown further below.
...
-- Q33) Average Population, GDP, and Schooling by Status (Snapshot)
